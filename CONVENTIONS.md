...
6.4 Shared client fetches follow the sanctioned data-hook pattern: a module-scoped cache plus a deduped in-flight promise, exposed as `use<Thing>()` returning `{ data, loading, error }`. `use-account-hierarchy.ts` and `use-carriers.ts` are the references. Re-run on a serialized key and guard async `setState` with an `active` flag.
...
8.1 Files and directories are kebab-case. Components are PascalCase. Hooks are `useCamelCase` in `use-*.ts` files. Types are PascalCase with a contextual suffix (`AccountPickerProps`, `CarrierOption`, `DashboardRunState`).
...
11.4 Inside the editing loop, run a scoped subset (`test:changed`, `test:related`); let the Lefthook pre-commit hook run the full gate. Prove changes pass before claiming done (CLAUDE.md 3.4).
