...
| `code-cadence` | `cadence_review`, `cadence_diagnose`, `cadence_implement` | First-party. Moves code that is technically fine but over-granular or non-idiomatic toward the simplest idiomatic form (React 19, v1). `review`/`diagnose` are static (ts-morph from `packages/ui`); `implement` applies a codemod then proves it via Biome/tsc/vitest, rolling back on failure. See [`code-cadence/README.md`](code-cadence/README.md). |
...
The `vitest`, `a11y`, and `code-cadence` servers are first-party source, not vendored
bundles: they have no upstream to regenerate from and use a minimal inline MCP stdio runtime
in place of the bundled SDK, edited directly here. Each splits its analysis into a sibling
module the `index.mjs` wires up (`vitest/analysis.mjs`, `a11y/analysis.mjs` + `a11y/audit.mjs`,
`code-cadence/analysis.mjs` + `code-cadence/verify.mjs`). `vitest` and `a11y` use only Node
built-ins; `code-cadence` loads the same way but its analysis resolves `ts-morph`/`typescript`
from `packages/ui`, so it needs the workspace installed.
