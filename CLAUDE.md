...
- **`claude/preferences.md`** — User code-style and formatting preferences. **At the start of every session**, read this file and apply its conventions to all code written. If the user states a preference (e.g., ordering, naming, formatting), append it here. If the file does not exist, create it with a `# Preferences` heading and organize entries under descriptive subheadings (e.g., `## Code ordering`, `## Naming conventions`). Each preference should be specific and actionable, and should note exceptions or conditions when they apply (e.g., "when possible", "unless dependency order requires otherwise").
