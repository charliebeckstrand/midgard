...
- When asked to create, add, write, or scaffold tests for anything in this monorepo — components, primitives, hooks, utilities, modules — in `packages/ui`, `packages/heimdall`, `packages/sindri`, or any app under `apps/`, always use the `/testing` skill. It detects the target's package and type, picks the right test patterns, and writes tests that match the codebase's exact formatting and conventions. The `/ui-component` skill invokes `/testing` automatically when creating a new component.
