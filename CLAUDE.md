...
- When asked to create, add, write, or scaffold tests for a component, primitive, or hook in `packages/ui`, always use the `/ui-testing` skill. It reads the source, determines which test patterns apply, and writes tests that match the codebase's exact formatting and conventions. The `/ui-component` skill invokes `/ui-testing` automatically when creating a new component.
- When asked to audit, review, or improve `packages/ui` at the design or architecture level — or asked about composition quality, recipe design, API consistency, or best practices — always use the `/ui-audit` skill.
- When asked to improve, clean up, or review code in `packages/ui` — or asked about code quality, formatting, naming, imports, stability, or consistency of component code — always use the `/ui-audit-code` skill. The `/ui-component` skill invokes `/ui-audit-code` automatically when creating a new component.
- When asked to perform an audit, design a complex feature, or make a high-stakes decision with multiple viable paths and real tradeoffs, always use the `/llm-council` skill. It runs the question through 5 independent advisors (Contrarian, First Principles Thinker, Expansionist, Outsider, Executor), peer-reviews their responses anonymously, and produces a chairman's verdict with a single concrete next step. Do not use the council for trivial yes/no questions, factual lookups, or casual decisions without meaningful tradeoffs.
