...
Produce a single canonical **Manifest** that downstream skills consume instead of re-deriving. The Manifest is a JSON document at `./manifest.json` — generated by this skill, then committed so downstream skills see a stable view across the team. Other skills read this file at the top of their flow and halt with a `run /repo:manifest` message if it's absent — they never generate it themselves.
