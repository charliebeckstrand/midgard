...
TRIGGER when: discover, profile, fingerprint, or summarize the project, stack, conventions, or workspace. Also invoked by `/postmortem` and `/premortem` to create or refresh `./manifest.json` when those lifecycle skills detect it missing or invalidated.

Produce a single canonical **Manifest** that downstream skills consume instead of re-deriving. The Manifest is a JSON document tracked at `./manifest.json` (committed to the repo). Other skills read this file at the top of their flow and halt with a `run /repo:manifest` message if it's absent — they never generate it themselves.

Optimized for a **Turborepo monorepo with Next.js apps and (optionally) shared React packages**. Degrades elsewhere: when `turbo.json` is absent, `monorepo.tool` is `null`; frameworks outside `next` / `react` resolve to `node` or `null`. Doesn't attempt to detect Nx/Lerna metadata.
...
Multiple lockfiles → pick the most recently modified; note the rejected lockfile names in `notes`.

**Workspace config** → confirm Turbo and capture the workspace globs:

- When `turbo.json` exists, set `monorepo.tool` to `turbo`. When missing, set to `null` and record a note — downstream skills that need richer Turbo metadata detect the absence themselves.
...
- A **vocabulary glossary**: project-specific terms (folder/recipe/primitive names) defined or repeatedly referenced, with a 1-sentence gloss inferred from context.
...
Per package, read `package.json` and glob 1-deep into `src/` (or the package root if no `src/`). Run packages in parallel.
...
Write pretty-printed to `./manifest.json` at the repo root. The file is tracked in git — when invoked from `/postmortem`'s diff-driven refresh, the caller is responsible for staging the updated file so it ships in the same commit as the change that invalidated it. Direct user invocations leave staging to the user.
...
- Don't interpret the manifest (e.g. recommend changes) here. Discovery is read-only.
- If the repo has zero packages with a `package.json`, emit a manifest with `packages: []` and a `notes` entry explaining why, then stop.
