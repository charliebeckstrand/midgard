...
| Path | Purpose |
|---|---|
| `auth` | Server-side gateway access: `bifrost` (gateway fetch), `getSession`, `requireSession`, `requireAdmin`, the helpers of the pending second sign-in step (`getSecondStep`, `requireSecondStep`, `forwardToSecondStep`), `getSignInProviders` (the GitHub and Google sign-in the gateway has set up), and the `Session`, `User`, `SecondFactorMethod`, and `SignInProvider` types. |
