...
import { IGNORED_PROPS, unaliasSymbol } from './ts-utils'
...
function buildPropDef(
	...
	// merged multi-arm props have no one structure and stay unclassified.
	if (propTypes.length === 1 && propTypes[0]) {
		prop.shape = classifyType(propTypes[0], checker)
	}
