...
| `ugoki` 動き | Motion — the `base` tempo primitives (`duration` / `ease`) every Framer Motion config composes from, the named `spring` vocabulary (`slide` / `settle` / `reflow` / `snap`), the shared data-viz `mark` reveal family, CSS transition fragments, and the enter/exit configs. |
