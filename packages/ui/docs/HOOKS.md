...
| `useVirtualWindow` | Drives a vertical windowed list off `@tanstack/react-virtual`: visible items plus top/bottom spacer heights, over uniform rows or, with a `getItemKey`, over measured rows. A measured list can hold its end in view. While the window is empty, the bottom spacer holds the height of every row, so a scroller that only `maxHeight` bounds grows to its cap. `scrollToIndex` takes an `align` and a `behavior`. |
