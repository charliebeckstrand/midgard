...
| `useVirtualWindow` | Drives a vertical windowed list off `@tanstack/react-virtual`: visible items plus top/bottom spacer heights; rows opt into per-row dynamic measurement via `measureElement`, and an `initialOffset` seeds the opening scroll position. |
