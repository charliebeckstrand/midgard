...
- **Cell-scoped sessions (shipped with this change).** `editable.scope: 'cell'` narrows a grid-owned session to the entered cell: the row still enters the set, but `GridEditingCell` mounts an editor only for the cell the session names (`activeEdit`, held beside the set and derived against it, so a declined or withdrawn row drops the coord). Entering another cell re-points the session — the cell it leaves commits on its own (`flushLeftCell`), and a previous row leaves the set — so every cell-scoped batch is one `CellChange`. Escape drops the active cell's draft alone, because the cells before it already committed. The setting needs the grid-owned session; under `'manual'` the consumer names a row and never a cell, so the row's editors all mount as under `'row'`. Default stays `'row'`.
