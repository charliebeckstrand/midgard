...
**The listbox editor has no keyboard commit path (2.1.1).** The boolean editor deliberately leaves Enter to the listbox's own open/select, and row scope covered that by mounting a sibling text or number editor to save from. Cell scope mounts one editor, so that sibling is gone. The value commits only by moving the session to another editable cell, which from inside the editor takes several Shift+Tab hops out through the header's resize and menu controls, unannounced. When the boolean column is the only editable one, the value is uncommittable by any route: `enterEditAtCell` refuses a non-editable column, so no move exists to flush it. The same gap covers an `editCell` slot that does not call `ctx.commit`. The increment-3 hoist of Enter onto the table's key surface is the fix; until then cell scope should not be recommended for a grid whose editable columns are all listbox-shaped.

**One Escape both closes a listbox panel and abandons the cell (2.1.1, and a data-loss surprise).** `sessionEscape` defers to an open floating surface by testing for a portal ancestor or an `aria-expanded="true"` ancestor. Neither holds for this repo's own `Listbox`: its panel renders inline in the cell rather than portaled, and the expanded trigger is the focused option's sibling, not its ancestor. So one press closes the panel and discards the edit. The regression test that covers the deferral passes only because it uses a static `aria-expanded="true"` button that never flips — the test is weaker than it reads.
