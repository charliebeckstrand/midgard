...
A follow-up closed the collapse and expand of groups above the viewport, such as the group menu's collapse-all and expand-all, which moved the view by those rows' height. The drop step moved into `useGridItemWindow`, and both bodies share it: every row of a collapsing group that does not animate stays one commit as a dropping item, goes to 0 px through `resizeItem`, and then leaves. A row under the sticky head now counts as above the viewport, in the body and in the hook's scroll adjustment, and so does an empty row that ends at the top edge. An insert above the viewport takes an anchor step: after the commit, the hook reads the new start of the row that was first in view through `getItemStart`, and moves the offset by the difference through `setScrollOffset`, which the virtualizer reads at once. A 0 px guess for inserted rows was tried first. It cascaded through the overscan, because rows of 0 px take no overscan distance, and it lost part of the adjustment. A browser case holds a collapse-all and an expand-all from the middle of 40 groups, and an expand-all of groups that never rendered, each to 1 px after paint.
