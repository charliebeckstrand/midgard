...
The collapse blocker is answered by keeping rows, not by holding them. A closing row that is in the window, a leaf or total of a collapsing group or a closing detail panel, stays as an item until their `transitionend`, with a one-second fallback for a row that leaves the window mid-tween. It takes a `closing:` key meanwhile, so its shrinking height does not overwrite the open height cached against the open key. The React key stays the open key, so the row keeps its node and its tween. Every other leaf leaves at once. An expand animates only the rows that fit in one viewport — `useGridRevealHold` gained an `enter` flag for a row that mounts open — and the browser test held the bound, so the instant fallback was not needed. A detail panel animates in view and opens or closes at once above it. A removal above the viewport is not a resize either; the start anchor in the follow-up below holds it.
...
A follow-up closed the collapse and expand of groups above the viewport, such as the group menu's collapse-all and expand-all, which moved the view by those rows' height. A row under the sticky head now counts as above the viewport, in the body and in the hook's scroll adjustment, and so does an empty row that ends at the top edge. A first fix used a drop step and an anchor step in the grid; a review folded both, and the 0 px detail guess, into one start anchor in `useVirtualWindow`. virtual-core 3.16 anchors only the end edge: `setOptions` records the key at the offset when the list changes, and its own layout effect applies the change in that row's start. The measured path holds the start edge the same way, in its own layout effect. Each commit records the rendered rows that end below the top edge, and a commit with a new count or key getter anchors to the first of them still in the list — a row re-keyed to `closing:` gives way to the next. A closing row guesses the height it had in view, so its new key moves nothing. Filling the library's own anchor step for the start edge was tried first and could not hold the tests: the library reads the new offset only from the next `scroll` event, a frame later, so a render in between painted the window at the old offset (276 px in the never-rendered case), and a second list change in that gap anchored against the stale offset. Sending a synthetic `scroll` event to close the gap looped in a scroll-up case and hung the browser suite. The hook therefore writes the new offset into the virtualizer as its scroll handler does, and renders again before the paint; the render after a move holds no anchor, so a move never starts another, and a scroller that does not scroll takes no change. A browser case holds a collapse-all and an expand-all from the middle of 40 groups, and an expand-all of groups that never rendered, each to 1 px after paint.

What stays out: manual grouping, and infinite scroll under either body. The cursor came back in #1269, except under manual grouping.
