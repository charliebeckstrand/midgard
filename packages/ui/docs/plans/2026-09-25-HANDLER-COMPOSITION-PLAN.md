...
The follow-up found the same defect in one more file, and a second change fixed it. `current-content.tsx` spread `(props as HTMLMotionProps<'div'>)`, and its props type accepted the four motion keys. The props type now omits them, and the element spreads `{...props}` with no cast. The type omits them in both modes, because the container sets the mode. `TabContent` and `NavContent` take their props from `CurrentContent`, so the omission applies to them too.

The same change taught the spread-order scan to read through a cast. `button.tsx`, `button-headless.tsx`, and `polymorphic.tsx` spread through one, and the scan found no violation there. A self-test proves that the scan reads a spread behind a cast, and it fails without the change.

The same change fixed the object-key half of §3.9 in `tab-contents.tsx`. `TabContent` wrote its probe ref before `{...props}`, and did not destructure `ref`. So a consumer's `ref` replaced the probe, and a panel with a focusable child kept `tabIndex={0}`. `TabContent` now merges the two refs with `useComposedRef`, as `ScrollArea` and `Sidebar` do. A test gives the panel a consumer `ref` and a button, and it failed before the change.
