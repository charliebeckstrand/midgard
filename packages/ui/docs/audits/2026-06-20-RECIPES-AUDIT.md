...
- **`kata/option.ts:20-24` and `kata/menu.ts:14-18` ◯ open.** Two sibling "selectable row" surfaces each hand-roll a per-step `gap`/`px`/`py` ladder, neither importing `ma`/`kasane`, and they diverge two ways: **different literal steps** (`option` `px-2 / 2.5 / 3` vs `menu` `px-2.5 / 3 / 3.5`) and **different axis modeling** (`menu` splits padding onto `density` and text onto `size`; `option` folds both onto `size`). The README remedy is a shared kiso `item-density` token — but the two ladders hold *different* values, so any shared token changes one component's rendered padding. **Left open for a design call** on which ladder is canonical (and whether `Option` should adopt `menu`'s density/size split).

**B. Size-driven `compound` matrices written out by hand (Medium). ✅ resolved (`6eeee4d`).** `kata/scroll-area.ts:16-22` already showed the idiom (`orientations.flatMap(o => sizes.map(s => …))`). Two kata wrote the equivalent longhand and now generate it:

- **`kata/button.ts:86-107`** — four `compound` rules for `bare × {xs,sm,md,lg}` collapsed to a `bareFloor` size→padding map driven by one `.map`. Literals stay literal for the scanner.
- **`kata/list.ts:51-64`** — 12 variant × density compounds (nine of them `class: p.{sm,md,lg}`) generated from `cardLikeVariants × densities → ma.p`, with the three `plain` exceptions kept explicit.
- ◯ **`kata/tabs.ts:62-69`**, **`kata/nav.ts:76-79`**, **`kata/sidebar.ts:76-80`** — lower-volume instances, left open (marginal; `tabs`/`nav` were under concurrent doc edits this pass).
...
| Bundle docblocks | kiso bundle `index.ts` files | omit the `Layer:` tag | ◯ open — minor; a README §2 note on the barrel exemption would close it |
...
| Compound-matrix idiom | B | A− | `button`/`list` generated; `tabs`/`nav`/`sidebar` remain longhand. |
...
| Naming / key ordering | A− | A− | Consistent typing; minor barrel/Layer-tag drift left (declined/open). |

**Done this pass:** B, C, D, E, F, and the resolved half of A plus most of G (`6eeee4d`, `9f5535c`). **Remaining (open):** `option.ts`/`menu.ts` ladder alignment (design call); the lower-volume `tabs`/`nav`/`sidebar` compound generation; the kiso bundle `Layer:` tag note. **Declined (rationale above):** `segment` return annotation, engine magic-string consts, barrel export ordering, `checkStandard` extraction. **Withdrawn:** `dialog.ts` config-key order (false positive).
