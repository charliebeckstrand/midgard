...
| JsonTree + Grid | `maxHeight` beside `virtualize` | json-tree.tsx:35-37 · grid mirror | Coupled pair enforced by a runtime `throw`; `virtualize` already takes an options object | `virtualize={{ maxHeight, … }}` in both — change both or neither | ◑ PARTIAL (JsonTree folded to `virtualize={{ maxHeight }}`; Grid half pending — conflicts with `implyVirtualize`, awaiting decision) |
