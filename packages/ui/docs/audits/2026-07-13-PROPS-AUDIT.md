...
|---|---|---|---|---|---|
| Control cascade | `invalid` vs `severity` | control.tsx:23 · use-control-props.ts:79 | Boolean duplicate of `severity="error"`; drift is live — `listbox.tsx:372` reads `control?.invalid` raw, so severity-marked Controls miss Listbox | Fold into `severity`; keep Input's leaf tri-state `invalid` (internally consumed); Listbox inconsistency fixes itself | ✅ RESOLVED (error severity now cascades to nested Controls, consistent with warning/success) |
| Dialog / Sheet / Drawer | `glass` vs leaked `surface` | dialog.tsx:26 + siblings | `useResolvedSurface(surface, glass)`; `surface=` passed nowhere; the glass-provider doc names `glass` the house convention | Drop `surface` from the public prop types (stays recipe-internal); resolution moves into the shared panel layer once | ✅ RESOLVED (`useResolvedSurface` simplified to `(glass)`) |
...
| Grid | top-level `sortable` vs `GridColumn.sortable` | grid-data-types.ts:693 | Default-flipper over a per-column flag — two sources of truth; the module's own demo misreads it (3 of 4 columns marked `sortable: true`, inert) | One source of truth: drop the grid-level flipper or make sorting per-column opt-in | ✅ RESOLVED (dropped the flipper; per-column `sortable` defaults true unless explicitly false) |
| GridInfiniteScroll | `showLoadingIndicator` + `loadingIndicator` | grid-data-types.ts:115,121 | Content prop silently inert unless the gate is also set | `loadingIndicator?: boolean \| ReactNode` (presence-implies, T5) | ✅ RESOLVED |
| GridColumnManager | `onSavePreset` payload | grid-data-types.ts:562 | Emits `GridColumnManagerPreset` — a second, narrower persistence vocabulary that can't round-trip into `GridPreferences` | Emit a `GridPreferences` snapshot; save and seed speak one type | ✅ RESOLVED (dropped `GridColumnManagerPreset`) |
| Chart (cartesian + scatter) | `grid` vs `axes.*.grid` | chart/engine/types.ts:307 | Whole-layer gate beside per-axis switches whose TSDoc must disambiguate it; never passed anywhere | Fold into the `axes` union | ✅ RESOLVED |
| Chart (cartesian) | `tickRotation` | chart/engine/types.ts:332 | Category-axis policy stranded at the frame level while its siblings live in `axes.x`; silent no-op when horizontal | Move into `ChartCategoryAxis` | ✅ RESOLVED |
| JsonTree + Grid | `maxHeight` beside `virtualize` | json-tree.tsx:35-37 · grid mirror | Coupled pair enforced by a runtime `throw`; `virtualize` already takes an options object | `virtualize={{ maxHeight, … }}` in both — change both or neither | ✅ RESOLVED (JsonTree folded to `virtualize={{ maxHeight }}`; Grid `maxHeight` kept top-level by decision — unlike JsonTree it also sizes the sticky-header / infinite-scroll scroll region, so it is a shared scroll-container prop, not virtualize-only) |
| PasswordConfirm | `onPasswordMatch` + `onPasswordMismatch` | password-confirm.tsx:17,20 | Two callbacks on opposite edges of one boolean | `onMatchChange?: (matched: boolean) => void` | ✅ RESOLVED |
| Listbox | `inputId` | listbox.tsx:46 | Same concept spelled `id` on Combobox and `htmlFor` on Field | Rename to `id` for family parity | ✅ RESOLVED |
| CommandPalette | `CommandPaletteEmpty` vs built-in empty `<output>` | slots.tsx:44 · command-palette.tsx:169 | Two empty-state mechanisms | Keep one; make the built-in text a slot or drop the component | ✅ RESOLVED (dropped `CommandPaletteEmpty`; built-in a11y `<output>` kept) |
| Motion provider | `reducedMotion` / the provider | providers/motion/motion.tsx:19 | Defaults duplicate the internal `ReducedMotion` primitive already applied at every motion root; only distinct value (`'always'`) has no consumer | Document "handled automatically"; deprecate the provider until `'always'` earns a spelled-out `<ForceReducedMotion>` | ✅ RESOLVED (removed the provider outright) |
...
| Button | `loading` object form | button.tsx:26,50 | `boolean \| {color,size,label}`; object branch passed nowhere including internally | `loading?: boolean` | ✅ RESOLVED (flattened to `loading?: boolean`) |
| Listbox / Combobox | `capitalize` object form | listbox.tsx:76 · combobox.tsx:95 | `boolean \| {displayValue?, options?}` plus two-context resolution plumbing, all so one boundary test can pass `false` | Collapse to `boolean`; delete `select-trigger/capitalize.ts` resolution | ✅ RESOLVED (collapsed to `boolean`; `SelectCapitalize` and `resolveCapitalize` deleted) |
| Container | `size` numeric branch | container.tsx:20 | Number → custom-property machinery; `size` never passed at all | Drop the numeric union; token axis stays | ✅ RESOLVED (numeric union arm and the `k.size.custom` recipe key dropped) |
| Grid | `groupTotalRow`, `grandTotalRow` | grid-data-types.ts:724,735 | Single-valued enums (`'bottom'`) — booleans in enum clothing | Booleans; widen when `'top'` exists | ✅ RESOLVED (both are booleans) |
| Grid | `header` config object | grid-data-types.ts:570,1022 | One field (`position`), one ever-passed value (`{position:'sticky'}` ×15) | `stickyHeader?: boolean` | ✅ RESOLVED (`stickyHeader?: boolean`; the `GridHeader` barrel export is gone) |
| Heatmap chart | inherited `animate`, `texture`, `subtitle` | heatmap-chart-schema.ts:61 · heatmap-chart.tsx:672-675 | Accepted publicly, destructured to `_discards`; a DOM leak-guard test papers over the type | Extend the existing `Omit<ChartBaseProps,'legend'>`; delete the discards and the leak-guard test | ✅ RESOLVED (the Omit covers all three; discards and the leak-guard test deleted) |
...
| Map barrel | `RangeArrow`, `RangeLegend` exports | map/index.ts:6-10 | `@internal` presentational parts exported publicly for one sibling consumer | Heatmap imports the leaf module directly (§3.5 allows it); drop from the barrel | ✅ RESOLVED (the slider moved to `chart/engine/chart-legend/range-legend.tsx`, so the heatmap imports it from its own module) |
| MapPlat | `deferPaint` | map-plat.tsx:103 | One caller (ChoroplethChart), one value (`true`) | Derive from `aspectRatio !== 'auto'` — the documented trigger condition | ◯ OPEN |
| Group | `as`/`href` polymorphism | group.tsx:26 | Never used; a joined-controls container as an anchor is an a11y trap the type invites | Plain `<div>` | ✅ RESOLVED (Group renders a plain `<div>`) |
| Collapse | `animate` `true` arm | collapse.tsx:23 | `true` is an alias of the default `'fade'`, special-cased back | `animate?: 'fade' \| 'slide' \| false` | ✅ RESOLVED (`animate?: 'fade' | 'slide' | false`) |
...
|---|---|---|---|
| SidebarLayout | `floating` | Textbook explicit-variant candidate, but its lone consumer toggles it at runtime (`floating={!locked}`) — a mode prop is defensible; owner's call | ✅ KEEP (a mode prop is right here: its consumer toggles it at runtime, `floating={!locked}`, which an explicit variant component cannot express) |
| Grid | `condensed` vs `density` | Boolean preset overriding the enum on the same axis (`condensed density="loose"` is legal-but-contradictory, test-pinned); deliberate design — fold into the density scale or document the orthogonality | ✅ KEEP, documented (orthogonal, not a step: `DensityLevel` maps 1:1 onto `Step` with no step below `sm`, and `condensed` also projects text/icon/badge classes, table-scoped. TSDoc now states it; closes API G18) |
| QueryBuilder | `allowGroups`/`hideFieldSelector`/`requireRule` | Only real consumer passes all three together as one "flat single-field filter" mode; a preset/variant could subsume the trio — deferred, the 2026-07-12 plan has this barrel mid-extraction | ✅ KEEP (the extraction landed; one composite consumer passes all three, and a preset would still thread three flags through context, so it buys nothing at one call site) |
| Drawer | `size` as Density `Step` | Same word, three meanings across the panel family (Dialog/Sheet: width variant; Drawer: density step); rename or let `createPanel` own the density story uniformly | ✅ RESOLVED — stale (O1 renamed the panel width axis `width`, so `size` is the density `Step` uniformly across Drawer, Popover, Tooltip, and Menu) |
| Menu | mode-by-prop-presence | `placement` present = dropdown, absent = context, `defaultOpen` = inline — three modes; ContextMenu already exists as one explicit variant, a `StaticMenu` would complete the set; noted as the family's next composition seam | ◯ OPEN — owner decision (highest-traffic compound; `StaticMenu`, documented presence, or an explicit `mode`) |
| Confirm | `confirm`/`cancel` bags | Bags are the right shape for a two-button wrapper; the real finding is adoption — admin hand-rolls a delete-confirm Dialog while Confirm sits unused | ✅ KEEP (the bags are the right shape for a two-button wrapper). The adoption claim is stale: `apps/places` uses `<Confirm>`. Re-recorded as one app migration — `apps/admin` still hand-rolls a delete Dialog |
| Sheet / Overlay | `modal={false}` + `backdrop` | The whole non-modal mode is test-borne; demo it or accept it as speculative surface | ✅ KEEP, narrowed (`modal={false}` now has two library consumers, so it is not speculative; only `backdrop` is still test-borne) |
| FiltersField | function-form `children` | Third wiring mechanism beside cloneElement sniffing and `useFilters()` — and arguably the sound one (cloneElement breaks on wrappers); pick a primary story | ◯ OPEN — owner decision (7 of 33 sites use the function form; the cloneElement path carries three identity tables that no-op on a wrapped control) |
| CalendarRange | `onValueChange` | Misnamed — reports the raw clicked day; the endpoint state machine lives in date-picker; document, or rename `onDayClick` at the next breaking pass | ✅ KEEP, documented (the TSDoc now names it an event reporting a raw click, against the sibling `Calendar.onValueChange` that echoes bound state; the rename waits for a breaking pass) |
| Input family | `data-group`/`data-group-orientation` | Necessary Group-stamp plumbing typed as ordinary public props across five components; mark `@internal` via one shared documented type | ◯ OPEN (eight declaration sites, not five; the fix is one `@internal` shared type intersected at each) |
| Grid | `rowClassName`, `rowLoading` | Zero/near-zero usage but genuinely useful shapes; keep, recorded | ✅ KEEP (both are §3.6 per-row render callbacks — deliberate architecture, recorded so the next audit does not relitigate) |
| Chart/map | `geographyObject`, `regionLabel`, `binning` | Real external-data variance, zero usage, and each must land in two prop vocabularies (MapPlat + ChoroplethChart renames) — demo-cover or consciously defer | ✅ KEEP (all three are live and consumed; `binning` reads through `useMapRegionReadout` off `props` — its destructure in `map-plat.tsx` is a DOM-leak guard, not a discard) |
...
| Tabs | manual `Tab id` + `TabPanels`/`TabPanel` wiring | Entire secondary linkage API appears only in tests; follow-up pruning candidate beyond a props audit | ✅ RESOLVED (`TabPanel` / `TabPanels` deleted — every consumer was its own test, while `TabContent` has 68 demo sites; the `spread-order-boundary` waiver retires with the file. `Tab id` stays as the escape for a panel the consumer renders) |
| ShinyText | nine tuning knobs | Demo-only decorative surface; policy call — demo-driven props default private until a consumer asks | ◯ OPEN — owner decision (now eight knobs; `delay` went and `direction` became `sweep`. The policy call is whether demo-only surface is public API) |
| PivotTable `totalLabel` · PasswordStrength `labels` | — | Unused, but each is its component's only i18n hook; resolve with T7's provider decision, not row by row | ◯ OPEN — gated on T7 (both are zero-usage and each is its component's only i18n hook; PasswordInput's toggle labels are now hardcoded with no prop at all) |
| `*-input` family | re-declared `prefix` | `Omit<X,'prefix'>` then type-identical re-declaration existing only to host a TSDoc default note (phone, zipcode, credit-card); a `@defaultValue` on the inherited prop does it in one place | ✅ KEEP — stale premise (each re-declares `prefix` to supply a default icon, `prefix ?? <Icon …>`; a `@defaultValue` on an inherited prop cannot do that. This is the default-then-overridable idiom F9 asks for) |
| Toast `position` · ToastProvider `duration`/`maxToasts` · CodeBlock `copy` · Combobox `closeOnSelect` · Dialog `placement` · CommandPalette `dismissOnBackdrop` | — | Coherent knobs whose only ever-observed value is the default; keep, recorded so growth is deliberate | ✅ KEEP — mostly stale (four of the six now carry non-default passes: `maxToasts`, `copy`, `dismissOnBackdrop`, `position`. `Dialog placement` no longer exists, since O2 renamed it `align`. Only `Combobox closeOnSelect` still has none) |
