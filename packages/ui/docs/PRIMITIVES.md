...
| `overlay` | Backdrop-and-panel shell for modal surfaces (Dialog, Sheet, Drawer) over a `PresencePortal`: focus trap, scroll lock, dismissal, dimming scrim. `reachable` names outside chrome that keeps its tab stop while the surface is up, for a long-lived work surface the trap would otherwise seal the user into; `overlayReachLayer` is the stacking rung that chrome must occupy. `elevated` is the inverse, for a surface that must paint over chrome the app lifted above the overlay root. | `Overlay`, `OverlayReach`, `overlayReachLayer`, `notifyOverlaySignal`, `subscribeOverlaySignal` |
