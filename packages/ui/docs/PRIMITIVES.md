...
| `panel` | Slot family + context envelope for panel surfaces; `createPanel` builds Title/Description/Header/Body/Footer/Content with Close and A11y contexts. `usePanelCloseValue` memoizes the root's `{ close }`; `usePanelCloseContext` reads it and throws outside a Dialog, Sheet, or Drawer. | `createPanel`, `PanelProviders`, `PanelClose`, `PanelTrigger`, `PanelCloseContext`, `usePanelCloseContext`, `usePanelCloseValue` |
