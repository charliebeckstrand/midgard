...
| `chrome` | Marks a region as application chrome no modal surface may seal off — it keeps its tab stop, its place in the accessibility tree, and its pointer events while the rest of the page seals (WCAG 2.1.1 / 2.4.3). Registration is by node, so no surface takes a prop and none has to name the region. | `PersistentChrome`, `registerChrome`, `chromeRegions` |
...
| `mount` | The shared hold behind every inactive panel. `useMountHold` resolves a `Mount` policy (`always`/`lazy`/`active`) into present, held, and hidden; `Hold` applies it through `<Activity>`; `heldMotionProps` builds the in-place motion props a held, animating panel spreads. | `useMountHold`, `Hold`, `heldMotionProps`, `Mount`, `MountHold`, `mountsEveryPanel` |
