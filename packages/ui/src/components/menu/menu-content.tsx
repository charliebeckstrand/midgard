...
export function MenuContent({
	...
	const viewport = (
		<div
			// Attached only while capped, which is the flag that emits the viewport's
			// `max-h`. An uncapped viewport has no height constraint, so it grows
			// with its rows and never overflows. The watch would then force a layout
			// read on the fresh node, and observe every row, to hold two attributes
			// that cannot change. `browser/menu-scroll-overflow.test.tsx` pins both
			// halves of that; `__benchmarks__/browser/README.md` §Menus prices it.
