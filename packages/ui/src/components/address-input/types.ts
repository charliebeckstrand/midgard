 ...
 * stores a match needs the pieces. A place is filtered by state, and an order is
 * shipped to a street. The only other way to the pieces is to parse the label
 * back apart.
 */
...
/**
 * A single geocoded match: a stable `id`, display `label`, and optional
 * `description`. It also carries the business or place `name` where the match is
 * one, the parted `address`, coordinates, and the provider's `raw` payload.
 */
export type AddressSuggestion = {
	 ...
	 * occurrence, and compares selections by the suggestion itself. A colliding id
	 * therefore costs a consumer their own storage, and nothing here.
	 */
	id: string
	label: string
	description?: string
	 ...
	 * searching by name reads the name back. A consumer storing the match keeps
	 * the name and the address apart.
	 */
	name?: string
