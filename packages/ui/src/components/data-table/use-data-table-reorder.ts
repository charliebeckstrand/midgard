...
/** Options for {@link useDataTableReorder}. @internal */
type DataTableReorderOptions<T> = {
 ...
 * {@link DataTable}. Only visible, non-pinned data columns are draggable, and a
 ...
 * @internal
 */
export function useDataTableReorder<T>({
	reorder,
	visibleColumns,
	reorderColumns,
}: DataTableReorderOptions<T>) {
	...
	const handleReorder = useCallback(
		(next: DataTableColumn<T>[]) => reorderColumns(next.map((col) => col.id)),
		[reorderColumns],
	)

	const { itemIds, strategy, dndContextProps } = useSortableList<DataTableColumn<T>>({
