...
[`pdf-viewer-open.bench.tsx`](pdf-viewer-open.bench.tsx) opens documents that [`pdf-fixtures.ts`](pdf-fixtures.ts) builds in memory: US-Letter invoice pages with 40 rows of Helvetica text and the rules between them, so no file and no network. Each `cold open` sample mounts `PdfViewer` in an 800 × 1000 box on a document the cache does not hold, and ends when the render queue is idle. The last sample of each page count prints the mean time to the first painted page. The `stage` scenarios split one page at 2x into the pdf.js render and the encode, with PNG beside the other encoders.

The bench loads the legacy build of pdf.js, as the viewer does. The modern build calls `Map.prototype.getOrInsertComputed` and other new built-ins, and the pinned Chromium (141) does not have them. The legacy worker goes in as the global `workerPort` and lives for the whole run. [`pdf-viewer-lifecycle.bench.tsx`](pdf-viewer-lifecycle.bench.tsx) measures the unpark of a resident document, with no pdf.js work.
...
2. **Measure the work, not the frames** ([`pdf-viewer-open.bench.tsx`](pdf-viewer-open.bench.tsx)). pdf.js waits for an animation frame before each 15 ms slice of a display render. The new `work only` rows answer each wait in a microtask. One 2x page is 17.2 ms as pdf.js paces it and 4.3 ms of work. A 0.2x thumbnail is the same 4.2 ms of work. The PNG encode, at 12.7 ms, is therefore three quarters of the work of a page. The flip row read 0.4 ms, but entry 4 found that it timed an empty viewer. Fourteen resident pages hold 2.74 MiB of PNG, and 58 MiB if each one decodes. The [raster plan](../../../docs/plans/2026-09-26-PDF-VIEWER-RASTER-PLAN.md) takes these numbers as its baseline.

3. **Open the document once and keep it** ([`use-pdf-viewer-document.ts`](../../components/pdf-viewer/use-pdf-viewer-document.ts), [`pdf-viewer-document-cache.ts`](../../components/pdf-viewer/pdf-viewer-document-cache.ts)). The open reads the size of each page and publishes a slot for each, before any page renders. The page count, the page navigation, and the highlight geometry are therefore whole at the open, and `onLoad` fires there. The cache keeps the pdf.js document for the life of its entry, so a later render needs no new parse. The first page now paints in 63 / 67 / 67 / 74 ms at 1 / 3 / 14 / 50 pages. The cost against entry 1 is the parse of the page sizes, about 6 ms at 50 pages. The settle does not change.

4. **Render on demand** ([`pdf-viewer-document-cache.ts`](../../components/pdf-viewer/pdf-viewer-document-cache.ts)). A queue for each document renders the page that a viewer shows, then its neighbors, then a thumbnail of each page at the width of the rail. A move cancels a full render that nobody wants now. A document keeps at most 8 full rasters, and the pages that a viewer wants. The open of a 50-page document now renders 2 full rasters, not 50. The first page still paints in about 60 to 90 ms, within the noise of entry 3. The settle at 50 pages is about 905 to 980 ms, against 1,119 to 1,341 ms, because a thumbnail costs less than a full page. Fourteen pages now hold 0.69 MiB of PNG with the thumbnails, where they held 2.74 MiB. A flip to a resident page costs 2.2 ms, and a flip to a page with no raster costs 47 ms, one render. The 0.4 ms flip of entry 2 was wrong. The body of its describe ran when the file loaded, and each cold open then reset the cache under it, so it timed a viewer with no page. The flip rows now mount their document in their first sample.

5. **Keep the bitmap, not a PNG** ([`use-pdf-viewer-document.ts`](../../components/pdf-viewer/use-pdf-viewer-document.ts), [`pdf-viewer-page-image.tsx`](../../components/pdf-viewer/pdf-viewer-page-image.tsx)). A full raster is now an `ImageBitmap`, and the viewport and the magnifier draw it on a canvas. The new `stage · bitmap` row takes about 0 ms, where the PNG encode took 12.7 ms. The first page now paints in 47 to 63 ms, and a flip to a page with no raster costs 20 to 24 ms, where it cost 47 ms. A bitmap is always decoded, so the cache holds at most 48 MiB of bitmaps across all documents. The least recently used document gives up its bitmaps first, and the pages that a viewer wants stay. A flip to a resident page costs 4.7 to 5.1 ms, against 2.2 ms, because the canvas draws the bitmap again on each flip. Thumbnails stay PNG, because the rail shows them in an `<img>`.

6. **Load the legacy build of pdf.js** ([`use-pdf-viewer-document.ts`](../../components/pdf-viewer/use-pdf-viewer-document.ts)). The modern build calls built-ins that need Chrome 147, Firefox 144 and Safari 26.2 (MDN compatibility data 8.1.3), far above the floor in `.browserslistrc`. The legacy build fills them, for about 58 KB more of minified page code and 50 KB more of worker code. The bench shows no cost: the first page paints in 44 to 49 ms, and a far flip costs 19 ms. [`pdf-viewer-browser-floor.test.tsx`](../../__tests__/browser/pdf-viewer-browser-floor.test.tsx) fails on the modern build in the pinned Chromium (141).

7. **Render only the thumbnails that the rail shows** ([`pdf-viewer-thumbnail-list.tsx`](../../components/pdf-viewer/pdf-viewer-thumbnail-list.tsx)). One `IntersectionObserver` reports the tiles in view, or within 200px of it, and the queue renders their thumbnails only. A closed rail, and the closed sheet on a phone, render none. The suite runs at a phone width (414 × 896), so the `cold open` rows now render no thumbnail: a 50-page document settles in about 100 ms, where it took about 900 ms. The new `desktop rail` row opens the same document at 1280 × 900. It renders 2 full rasters and 4 thumbnails, and settles in about 190 ms. After that, a reader on one page causes no render.

8. **Render the page again when it shows larger** ([`pdf-viewer-document-cache.ts`](../../components/pdf-viewer/pdf-viewer-document-cache.ts)). The viewer gives the queue the width at which it shows its page. When the page shows wider than its slot, the queue renders it again at a larger scale, after the slot raster of the page and before its neighbors. So the first paint does not wait for it. The scale goes up in steps of a quarter, so a resize renders again only at a step. A sharp raster holds at most 8 MiP, which is 32 MiB. The new `zoom 2` row opens one page at a zoom of 2: the first page paints in about 39 ms, and the sharp raster, 1,606 pixels wide against 918, lands by about 78 ms. At a zoom of 1 the page fits its slot, and the other rows render nothing more.
