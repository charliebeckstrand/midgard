 ...
 * because the element unmounts while closed. The raw flag is therefore re-read on every
 * reopen within the same mount. A bottom drawer that arrived open from `?view=`,
 * minimized, then maximized would stand back up in place instead of sliding.
 *
