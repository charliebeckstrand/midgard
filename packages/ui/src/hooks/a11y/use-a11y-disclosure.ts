...
export type A11yDisclosureOptions = {
	 ...
	 * trigger and panel rendered as separate components must derive matching ids
	 * — combine it with `key` to discriminate each pair.
	 */
	id?: string
 ...
 *
 * Call it once where both ids are visible to both sides — typically the
 * component's context provider — or, when the trigger and panel are independent
 * components, call it in each with a shared `id` plus a per-pair `key`; the ids
 * derive purely from those, so the two calls agree.
 */
