 ...
 *
 * The pin is written straight to the element rather than through a render. That
 * is what makes the origin reachable at all. A `ResizeObserver` callback lands
 * after layout and before paint, so a pin written from one reaches no frame of
 * its own. A render that owned the in-flight height would stamp the resting
 * value back over it.
 *
 * The inline height is cleared on arrival, so the resting size goes back to
 * being CSS's. The cap a class states therefore keeps bounding the box, with no
 * length restated here. It is deliberately *not* cleared when the caller stops
 * the returned controls. An interrupted travel leaves the box where the last
 * frame put it. A caller that hands the height to something else needs exactly
 * that.
 *
