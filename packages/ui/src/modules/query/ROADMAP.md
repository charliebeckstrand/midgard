...
A rule with an unknown operator now reads as inactive. Before, `isQueryActive` resolved the operator from the field set. So a rule with an operator that the evaluator does not know read as active. The summary and the chips showed it, but the rows did not change. A value-less operator that the field did not offer read as inactive, but the evaluator applied it. Now one judgement, `imposesConstraint` in `engine/query-evaluate.ts`, decides for all four readers. A rule is active when the evaluator has a matcher for its operator, and the operator reads no value or its value is filled. The judgement reads no field set, so `isQueryActive` takes only the group.
...
The summary now names an operator that the field does not offer. Before, [`describeRule`](engine/query-summary.ts) read the label only from the operator set of the field. A blank dashboard selection makes an `isEmpty` rule on a field of each type, so a `number` field showed the raw `isEmpty`. Now the label comes from the set of the field, then from the default set of the field type, then from the other built-in sets. So `isEmpty` on a `number` field reads `is Empty`, and `equals` on a `date` field with custom operators reads `on`. A parity test holds the evaluator operators and the built-in sets equal, so no active rule shows a raw operator name.
