...
A tile takes a `mount` policy, so a long board can hold back the content of the tiles under the fold. A tile also takes a `defaultSize`, the span that it takes before the layout holds an entry for it. The tiles with no entry take their new rows in markup order: a JSX tile by its place among the children, and a spec tile by its place in the spec. The tiles that one component renders take their rows in mount order. A reload therefore gives each of them the same row.
