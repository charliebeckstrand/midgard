...
Regions colour by category from typed rows; unmatched regions take the neutral no-data fill. Overlays register their own legend entries through context, so the one legend switches regions and routes alike — pointing dims everything outside the focused group, clicking toggles — and pointing a region or overlay on the map isolates it behind the same recede, the chart module's pointed-mark emphasis. The tooltip is the real Tooltip component (`TooltipContent` on the map's own floating state), anchored to the pointer and flipping at the viewport edges. `onRegionClick` turns the regions into a picker — the charts' `onCategoryClick` in the same shape — reporting the clicked one's identity so a click keys straight into the caller's rows; it is a pointer enhancement over a control the consumer supplies, since the paths stay presentational inside the `role="img"` plot. `selectedRegion` draws the other half of that picker: the pick comes back by the same identity and rings above every layer, outlasting the passing hover emphasis, and its row in the data table reads as the current one. The map holds no selection state of its own, so the map and the control beside it read one value and can never disagree about what is picked. Routing clients return street geometry plus distance and duration for mileage readouts; geocoding stays with `AddressInput`, whose suggestions carry the lon/lat an overlay consumes.
