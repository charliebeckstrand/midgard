...
- **A selected overlay.** `selectedRegion` rings a region and marks its table row `aria-current`. An overlay pick shows nowhere: `MapTableProps.selected` is a region index, and the plat has no selected-overlay concept. The table now carries every mark and every dot through one readout resolver, so the rows a pick would mark exist — only the selection does not reach them.
