/**
 * The one map from a mark's shape to the `Swatch` that stands for it in the
 * readouts. The legend and the tooltip both key a swatch off a mark, and
...
/**
 * The `Swatch` shape each mark shape reads out as: a square for the geofence,
 * the one mark that draws an area — and for a region, which is an area too
 * though it is no overlay — a line for a route and a marker, a circle for a
 * point and for a set of them.
 *
 * @internal
 */
