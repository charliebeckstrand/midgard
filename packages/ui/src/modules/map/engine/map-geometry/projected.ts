 ...
 * the same geography under two fits in one mount. One is canonical, on the first
 * commit; the other is measured, a beat later. It draws it again under a third
 * fit on every resize. `regionPaths` streams every coordinate through `d3-geo`
 * afresh for each. That walk is the mount's largest pass: 243 ms across 3,108 counties,
 ...
 * coordinates. {@link emitRegionPaths} then writes the `d` strings from that
 * buffer under any later fit. The later fit is reached by arithmetic, because a
 * d3 projection places a point as `scale · raw + translate` and nothing else.
 * Two fits of one projection therefore differ by a scale and a translation on
 * the frame. That is the same fact `scaleCanonicalFit` (`map-projection/fit`)
 * already derives the measured fit itself from. The claim is tested rather than
 * trusted. The buffer keeps one witness position. A fit that does not place it
 * where the affine says refuses the buffer, instead of drawing from it
 * ({@link agrees}).
 *
 ...
 *
 * What parts is the density of points along a curve, never where one lands. The
 * gap therefore reads as a straighter long edge, and is bounded by the tolerance
 * the fit it was drawn at already accepted. A map meets it only by pairing
 * hand-drawn geography with a frame far from `MAP_CANONICAL_WIDTH`, and the
 * witness above cannot see it. A resampled vertex is on the same affine as every
 * other. That is why the bound is stated here and pinned by test, rather than
 * checked at runtime.
 *
 ...
 * explicit `clipExtent` cuts in frame units that do not scale with it. The same
 * buffer read at two scales would then keep two different pieces of the
 ...
 * to end. It carries the index each region's rings start at, and the fit the
 * points were measured under.
 *
 * Flat typed arrays rather than nested ones, because the whole point is to hold
 * a county atlas cheaply. Here 63,888 points is one 1 MiB buffer, against 3,108
 ...
 * unclosed run. Either would therefore come back from the emit as a closed ring
 * through its vertices — a different shape, not a rounder one. A collection nests
 ...
 * scaled. The geography it keeps is therefore a different piece of the map at
 * every fit. The buffer instead holds the one piece it was drawn with. The
 * built-in projections all report none. `albers-usa` carries no `clipExtent`
 * method at all, and `mercator` and `equal-earth` default to `null`. The
 * world-square clip `mercator` holds internally is re-derived from the scale on
 * every change. It therefore cuts the same ground at every fit, and never
 * surfaces here.
 *
 * @internal
 */
 ...
 * arithmetic's own drift measures ~1e-13. This therefore sits far under what
 * could show, and far over what floating point costs.
 */
 ...
 * claim. Each would leave `scale` and `translate` readable and plausible. Placing
 * one known position through `fitted` and comparing costs a single projection
 * call against a pass of tens of thousands. It turns every one of those into a
 ...
 * `moveTo` at each ring's first point and a `lineTo` along the rest. A ring
 * boundary is therefore exactly a `moveTo`, and needs no other mark.
 *
 * `beginPath`, `closePath`, and `arc` are the interface's, and only the first
 * two are ever reached. {@link areaOnly} keeps every geometry that would draw an
 * arc off this path. A ring's close needs no mark, because its end is the next
 * ring's start.
 *
 ...
 * walk. That is a geography of more than area rings, or a projection whose
 * `clipExtent` would cut a different piece of it at each fit. A caller that
 ...
 * is then read from it several times that size. `geoPath` refines a curve
 * against a tolerance in frame units. A buffer taken at d3's default would
 * therefore carry the probe frame's idea of a curve into every fit above it.
 * Refining by this factor puts the buffer at or past what the canonical frame
 ...
 * projection {@link projectAtlas} declines, or bounds that collapse. The caller
 * then falls back to `canonicalFit` (`map-projection/fit`), which measures its
 ...
 * its way. Across 3,108 counties, 163.2 ms and 202.9 ms become 167.7 ms. The
 ...
 * the affine multiply is 1.4 ms and the concatenation 0.8 ms. The remaining
 ...
 * `null` where it cannot be carried there. That is a projection that gained a
 ...
 * The region layer applies it as one group transform, to draw the paths it
 ...
 * per refit. It used to take every path back through the projection and rewrite
 * every `d`. That was 47.1 ms of writes and forced layout across 3,107 counties,
 * against 0.15 ms. It is the module's own {@link MapTransform}, so the layer
 * renders it through `transformAttribute` (`map-zoom/transform`). The two
 * transforms this module puts on nested groups are therefore written by one
 * hand, and cannot drift on the attribute's format.
 *
 * Neither affine off the buffer is the one the layer needs, because the buffer
 * is held at the probe scale the frame was measured from. This composes them. A
 * buffer point reaches the canonical frame as `p·fc + dc`, and the measured one
 * as `p·fm + dm`. The canonical frame therefore reaches the measured one by
 * `fm / fc` and the offset that leaves. Both halves are witness-tested, so the
 * composition is too.
 *
 ...
 * same object at the same scale by the time this runs. The basis those paths
 * were drawn at is gone. Named projections mint a fresh instance per fit
 ...
 * index-aligned with the features it was drawn from. It is `null` where a
 * feature drew no ring. It is `null` for the whole atlas where `fitted` cannot
 * be reached from the buffer's own fit. That is a projection that gained a
 * `clipExtent`, one scaled to nothing, or one that fails the witness
 * ({@link agrees}). The
 * caller then falls back to the direct walk, as it does for a buffer that never
 * built.
 *
 * The output is `geoPath`'s own. `M` goes at each ring's first point, `L` along
 * the rest, and `Z` to close it, each coordinate rounded the way
 * `geoPath.digits` rounds it. Rings are what a region is here
 * ({@link areaOnly}), so every one of them closes. Byte for byte up to the frame
 * the buffer was drawn at, for the reason
