 ...
 *   as its region's complement and floods the frame.
 * - The degenerate rings every source carries, which no winding saves.
 *
 * They were once one question, both read off a ring's spherical area. They are
 * not any more. {@link planarSide} settles the winding of 99.25% of rings on the
 * plane, and never measures the sphere. The drop stays on the exact measure,
 * whatever the plane reports. What now couples them is
 ...
 * the cached decode stage (`cache.ts`). It costs 1.6 ms across the 3,231
 ...
 * `min(area, SPHERE − area)` collapses toward zero. It sits well above the
 * floating residue such a ring leaves (~1e-13), and well below the smallest
 * genuine region. A 1 km² sliver spans ~2.5e-8 sr. It therefore parts degenerate
 * rings from tiny real ones.
 *
 * @internal
 */
 ...
 * to a lower bound on the spherical one, rather than to the figure itself. A
 * ring near the epsilon therefore needs the exact measure, and this band is
 * where "near" ends.
 *
 * A hundred rather than a thousand. The fallback stays free at this width: 0.71%
 * of `counties-10m` rings, 0.33% of `states-10m`, 1.75% of `countries-110m`. A
 * thousand swallows 5.41% and 38.69%, and the pass stops being faster than the
 * one it replaces.
 *
 * @internal
 */
 ...
 * inside a narrow band of longitude. For those the planar shoelace decides the
 * same question by arithmetic. It settles 99.25% of `counties-10m`, and the
 ...
 * homeomorphism, and the enclosed side spans at most `2w < 2π`. The sign of the
 * planar area is therefore the side of the sphere. A ring can never measure
 ...
 * shoelaces are small and 15 positive ones are large. There are no exceptions
 * either way.
 *
 * The one shape this misreads is a ring that crosses itself. There the shoelace
 ...
 * atlases ship them anyway. Across every file `us-atlas` and `world-atlas`
 * publish, 141 of the 22,285 rings cross themselves, five of them in
 * `counties-10m`. What makes it safe is that the sign only turns over when two
 * oppositely wound lobes are comparable in area. A coastline that nicks itself
 * is a spur against a landmass. Every one of those 141 agrees with the spherical
 * measure. That is a measurement over the atlases a consumer is likely to draw,
 * not a proof. It is therefore the reason to keep the byte-for-byte comparison
 * in reach, rather than to stop checking.
 *
 * @internal
 */
 ...
 * parallel measures exactly 0 on the plane and 4.6e-6 sr on the sphere. That is
 * four thousand times the epsilon, and d3 draws it.
 *
 * @internal
 */
 ...
 * wound wrongly under neither reading. That is the third outcome the plane can
 * never reach, and the reason {@link planarSide} refuses the one span that can.
 */
 ...
 * reversed, and each hole is set to the opposite winding. A backwards exterior
 * reads to d3 as the region's complement, flooding the frame (and, under
 * `albers-usa`, its inset clips). Collinear zero-area rings, junk under either
 * winding, are dropped; a polygon whose exterior is one is dropped whole.
 *
 ...
 * visible effect is small. One `counties-10m` feature is left with nothing to
 * draw. The 89 others that draw no path are territories `albers-usa` places
 * nowhere. The drop earns the pass on the holes rather than on the count.
 * {@link DEGENERATE_AREA_EPSILON} states why a ring of no area cannot be wound
 * either way. A hole flipped on that reading punches out the region it
 * belongs to.
 *
