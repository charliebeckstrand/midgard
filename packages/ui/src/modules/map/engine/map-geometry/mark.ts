/**
 * What the overlay marks draw:
 *
 * - One lon/lat projected to the frame and back.
 * - The path strings a dot, a polyline, and a closed ring paint from it.
 * - The anchor each shape offers the keyboard cursor.
 * - How much room an area's own face holds.
 *
 * Held apart from `region.ts` because the marks project point by point through a
 * closure the plat hands down. The region layer instead projects whole features
 * through `d3-geo`'s own path generator.
 *
 * The area primitives — `featureRings`, `projectArea`, `areaReach`, `boxNear`,
 * `ringsNear` — now answer three subsystems rather than one:
 *
 * - The geofence that first wanted them.
 * - The ZIP dissolve.
 * - The region half of the hit-target rule.
 *
 * The last two draw no mark. They stay here rather than moving to an `area.ts`,
 * because each is built on a private walk the marks share. `projectArea` rides
 * `projectRun`, `ringsPath` rides `polylineCommands`, and `areaAnchor` rides
 * `ringAnchor`. The split would import back into this file for
 * its own internals, and buy a boundary that does not hold.
 *
 ...
 * either way, and one that wraps the world does not. The marks therefore keep
 * the cheap walk, and the region layer keeps the exact one.
 */
 ...
 * from, or `null` where the fit has none. That is an unfitted projection, a
 * point the composite drops between its insets, or one a projection cannot
 * invert at all.
 *
 * @internal
 */
 ...
 * geometry collection. Every reader here already spells that as no room.
 *
 ...
 * middle points. The common two-point line therefore anchors between its ends,
 * rather than on one of them. That is a `MapMarker` with no routed path, or a two-stop
 * `MapRoute`. Interpolated in lon/lat rather than on the projected plane: the
 * anchor only has to sit on the mark. This way it needs no projection to
 * compute, and survives every refit.
 *
 * @internal
 */
 ...
 * before it walks a ring's edges. That is how a zone tells a dot standing
...
/**
 * Whether one box holds a position once grown by `margin`. It is the test
 * {@link ringsNear} runs per ring. It is stated once, so a caller holding a
 * single box can ask it without wrapping that box in a list.
 *
 * @internal
 */
...
/**
 * Whether any of an area's rings draws within `margin` frame units of a
 * position. It is the cheap reject, on the boxes {@link projectArea} already
 * measured, before anything walks an edge. It is how a zone tells a dot standing
 * nowhere near it from one competing for its ground.
 *
 ...
 * points the projector drops left out. Any ring left under three of them is
 * dropped whole, three being the fewest an area can hold. An inset that clips
 * half a territory away therefore keeps the half it kept.
 *
 ...
 *
 * Each ring carries its own bounds, measured on the walk that collects it. That
 * is free here, and what lets a zone reject a dot it stands nowhere near without
 * touching an edge of it.
 *
 * @internal
 */
 ...
 * boundary. It is the radius of the largest circle the shape holds, near enough.
 * It is `2 · area / perimeter`, which is that radius exactly for a circle (the
 * form `circleRing` draws, and so most zones on a map). It reads a long thin
 * corridor as its width rather than its diagonal. That is where a bounding box
 ...
 *
 * Every ring counts, holes included. A zone drawn as a band around a large hole
 * therefore reads the room its outer ring would hold, rather than the band's own
 * width. The cause is the shape this reads, not the winding. {@link projectArea}
 * flattens the polygons to one ring list, because {@link ringsPath} needs no more
 * than that. Which ring opened a polygon is therefore gone before this measures.
 * Keeping that grouping is what a band would need: the outer ring's area less
 * its holes', over the whole boundary. It costs the flat list every other
 * reader here wants. The reading errs generous, so the dot keeps more of its
 * target rather than less. A zone of that shape is rare enough to leave the
 * grouping unbuilt.
 *
 * @param rings - The projected rings, from {@link projectArea}.
 ...
 * has no middle that stands on it. The cursor must land on the mark, and not
 * between its pieces. Size is read as the enclosed area of the outer ring, and
 * read winding-blind. A backwards ring measures as the whole sphere less its own
 * area, so the smaller of the two readings is the one the ring means. A
 * dissolved shape therefore needs no rewinding pass to be measured.
 *
 * @internal
 */
 ...
 *
 * Drawn as a stroke — not a `<circle>` — because only a stroke width could ride
 * device pixels through `vector-effect="non-scaling-stroke"`. The marks size
 * themselves now (`MapDot`), so the two shapes would draw alike. Swapping still
 * moves the pop-in, the halo, and the hit geometry with it.
 *
 * @internal
 */
 ...
 * builder in `map-cluster/ground.ts` both put frame ordinates into the DOM. One copy each is one
