 ...
 * for, and a mark the legend has toggled off never become stops. The cursor
 * therefore steps geography and overlays as one field, the way the pointer
 * crosses them, and only ever stands where the map draws.
 *
 ...
 * follow.
 *
 * Three things keep a stop out, and each is a place the map draws nothing:
 *
 * - a region whose geometry carries no centroid
 * - a mark the projection has no image for (the US composite drops points
 *   outside its insets)
 * - a mark the legend has toggled off, which unmounts its shapes while its
 *   registration stands
 *
 * A
