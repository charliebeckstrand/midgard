 ...
 * depth is the only thing that parts it from a single ring. Two public surfaces
 * also hand it to each other: `useMapCoverage` returns one and `MapGeofence`
 * takes one. A reader who counts the brackets wrong on either side type-checks
 * and draws nothing.
 */
...
/**
 * A swatch shape, mirroring the mark it stands for:
 *
 * - `rect` for the two things that draw an area: a region and a geofence.
 * - `line` for a route and a marker.
 * - `dot` for a point.
 *
 ...
 * object and a GeoJSON feature both hold an `id` and `properties`. That is what
 ...
 * and which of its stops. The stop is a `MapPoints` dot's index in the points
 * the caller passed. Omitting the index names the mark's first stop, which is
 * the whole of a singular mark. An index the mark has no stop for picks
 * nothing, the same silence an id naming no mark keeps.
 */
...
/**
 * The map frame's aspect ratio:
 *
 * - `'auto'` takes the fitted geography's own projected proportions.
 * - A `width / height` number or `"4/3"` string fixes one.
