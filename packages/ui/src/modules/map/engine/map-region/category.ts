/**
 * Pure category plumbing for the map module:
 *
 * - Deriving the categorical series from the rows.
 * - Resolving slot paints in the fixed order.
 * - Matching each region to its row's category.
 *
 * The map's analogue of the chart module's series resolution, and the shape its
 * numeric twin (`value.ts`) emits too. The regions, the legend, the tooltip,
 * and the table therefore read either mode unchanged.
 */
 ...
 * `value`, not its position. Keying on the value ties a toggled-off entry to
 * the same category when the resolved order shifts. That order shifts on a data
 * update reordering the first-appearance derivation, or a category dropping
 * out. A positional token would instead re-point at whatever now sits at that
 * index. A numeric bin's `value` is its index, so its key is unchanged.
 *
 * @internal
 */
 ...
 *
 * Public because a legend rendered OUTSIDE the plats it drives (one bar
 * standing for several maps) has to name the bin itself. The id scheme is this
 * module's to keep. A numeric bin's category value is its index, so this is
