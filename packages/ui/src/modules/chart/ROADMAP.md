...
Three rungs, coarsest to finest. The **competitive browser suite** (`pnpm bench:browser`, real Chromium, production React) keeps score against the market — mount, update, and hover across line/bar/scatter at 50 to 10,000 points. The **jsdom benches** (`pnpm bench`) localize a regression to the module without a browser — `chart-resize` proves the ResizeObserver path stays one bounded rebuild per notification with no host cascade. The **pure cores** are the finest rung and the one gap in the ladder: `chart-scale`, `chart-layout`, and the per-chart `*-geometry` builders have correctness suites but no microbenchmark, so an optimization to `lineGeometry` or `linearScale` is today measured only end-to-end, where React reconciliation and DOM commit dominate and can mask a real core win or loss. Closing that gap (below) is the first item, because every avenue after it wants a core-level number to hold the line.
...
### 1. Pure-core microbenchmarks (the prerequisite)

Add a jsdom `chart-geometry.bench.ts` and `chart-scale.bench.ts` beside the existing benches, exercising `lineGeometry`, `barMarks`, `scatterGeometry`, and `linearScale` at 1k/10k directly — no React, no DOM. This turns every avenue below from an end-to-end guess into a measured core delta, and gives the string- and allocation-level changes (2–4) a number that the browser suite is too coarse to show. It ships nothing to users; it is pure instrumentation, and it is first because the rest lean on it.
