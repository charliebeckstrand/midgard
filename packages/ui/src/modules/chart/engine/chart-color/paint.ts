 ...
 *   `className`;
 * - a *raw* CSS colour (a hex, `oklch()`, or any value CSS accepts), carrying
 *   one inline value.
 *
 ...
 * classes. It can instead be a raw CSS colour (a hex, `oklch()`, or any value
 * CSS accepts) applied inline. The generic input the whole colour system resolves from —
 ...
 * colour carries one inline value, applied as an SVG paint attribute (`fill` /
 * `stroke`) or the swatch's `currentColor`, and absent for slots. One union, so
 * every mark, swatch, and label paints a slot and a raw colour the same way. It
