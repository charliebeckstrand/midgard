 ...
 * {@link chartFramePolicy} pipeline at each measurement — every stage pure, so
 * a candidate size resolves without a render — and compares only the discrete
 * grants. The tick target (a gridline per ~44px of height) and the compact
 * number format are left out: the first would flag a flush every few rows of
 * drag for a one-hairline change, and the second flips only at the compact
 * width the band-axis mode already crosses.
 *
 ...
 * `null` / `undefined` otherwise — as handed to {@link chartFramePolicy}.
 * @param chrome - The header and stacked legend around the plot — as handed to
 * {@link chartFramePolicy}.
