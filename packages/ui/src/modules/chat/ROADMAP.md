...
The proof under both is in [`chat-stream.test.ts`](../../__tests__/modules/chat-stream.test.ts) and [`chat-embed.test.tsx`](../../__tests__/modules/chat-embed.test.tsx), and the module now holds 13 test files. It still holds no benchmark, which increment 7 owns.
