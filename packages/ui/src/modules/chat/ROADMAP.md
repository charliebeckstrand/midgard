...
The seam is open. Increment 4 made `embed` the union's second kind and put [`ChatEmbedProvider`](chat-embed-provider.tsx) over a record of renderers by name, so a reply can carry a chart, a grid, or a map while the module imports none of the three. A name no renderer claims states itself in the bubble rather than drawing nothing. What the seam does not have yet is a stream: [`ChatTransport`](use-chat-send.ts) still yields a string, so an embed reaches a message only through `initialMessages` or `setMessages` until increment 5 widens it.
