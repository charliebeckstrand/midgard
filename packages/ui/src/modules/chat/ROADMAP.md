...
One cost the audit did not predict. JSX reserves `role` for the ARIA attribute, so Biome's `useValidAriaRole` reads `<ChatMessage role="assistant">` as an invalid ARIA role and fails the lint. Every other `role` in the package is a real ARIA role, so chat is the first prop to take the name for anything else. The fix is `ignoreNonDom` in [`biome.json`](../../../../../biome.json), which holds the rule at full strength on DOM elements and drops it on components, where a prop named `role` is only a prop and the rule cannot know the contract. TypeScript still checks the pass-through props that do carry an ARIA role (`Dialog`, `primitives/popover`). A caller who lints with the same rule needs the same setting.
