/**
 * Pure prop/config resolvers split out of {@link GridData}. Each collapses one
 * piece of the grid's configuration into the resolved shape head, body, and the
 * `<table>` read. That piece is a prop shorthand, the rendered-window mode, or
 * the resize layout. The branching therefore stays off {@link GridData}'s
 * cognitive-complexity budget, and open to direct unit testing. Kept a `.ts`
 * utility module (per the filename
 ...
 * `totalRows`, else on outright (the consumer stops it). `loadingMore` defaults
 * off. `threshold` falls back to the virtualization `overscan`, so the fetch
 * leads the viewport by about the windowed overscan. The loading indicator
...
/**
 * The grid's `aria-rowcount`:
 *
 * - The server total + 1 when paginating with a known total, or
 *   infinite-scrolling with a known {@link GridInfiniteScroll.totalRows}.
 * - The rendered count + 1 when unpaginated.
 * - ARIA's `-1` "indeterminate" sentinel when the whole extent is unknown.
 *
 * The extent is unknown for a server feed paginating by `pageCount` alone, or
 * infinite scroll with more rows to load and no stated total. The sentinel
 * beats misreporting the loaded window as the whole set. `extraHeaderRows` adds
 * any header rows beyond the column header — the column-group band row — so the
 * count spans both. The indeterminate sentinel is left untouched.
 *
 * @internal
 */
 ...
 * "all rows". A plain, non-navigable table conveys all this natively and stays
 * a table. Manual grouping withholds the semantics outright. Its body
 * interleaves group-header and leaf rows with no global index bookkeeping (the
 * cursor and virtualization already stand down under it). Even a manually
 * paginated grouped grid therefore stays a native table, rather than
 * advertising indices it doesn't emit.
 *
 * @internal
 */
 ...
 *
 * The two differ on one case: a view emptied by a filter or a search. That is
 * not a grid with nothing to act on. The rule that emptied it lives in the
 * header. The header's affordances (sort toggles, funnels, the right-click
 * menu) are therefore the only way back to the rows, and have to stay live.
 * Client-side, `rows` still holds the source and the case never arose.
 * Server-side ({@link GridColumnFilters.manual}), the consumer feeds back the *filtered*
 * set, so an over-narrow rule leaves zero rows here. Standing the header down
 * then strands the user in a view they cannot undo. Row-level actions still
 * read `hasRows`: dragging a row needs a row.
 *
 * @internal
 */
