...
6. **Measure churn, not just wall time — stability is a GC story.** The second goal is steadiness, and jank is allocation-driven: decorate-per-sort and per-render array building drive the pauses. Extend the node benches with allocation-count assertions — sort and refresh cost O(page), not O(total), as the loaded set grows — so a regression in churn is caught as a number before it surfaces as a stutter. This is the guardrail the other avenues are proven against.

7. **Don't re-render the un-visited cell on resize.** A new bench (`grid-resize-truncation.bench.tsx`) put a number on a real cost: on an un-windowed grid, a column resize that mass-truncates its cells runs ~2× the cost of the same grid with `truncate={false}` (1,000 rows, 118 ms vs 60 ms; 3,000 rows, 223 ms vs 148 ms). The overflow *read* is already contact-gated — an un-visited cell's `measure` bails — but the grid still re-renders every cell in the resized column through its per-column settle key to re-measure, and for an un-visited cell that re-render produces the identical bare span it already showed. *Shipped:* the deferred re-measure backstop (`useGridTruncation`) now gates on `contacted`, so an un-visited cell no longer queues a per-resize no-op frame — thousands fewer rAF closures per resize (a churn win), though the wall-time residual is the re-render itself, not the frame. *Open, and load-bearing:* suppressing the un-visited cell's re-render means the settle key can't be a plain prop on the memoized cell — the cell would subscribe to its column's settle width only once armed, a change against the resize-measure path a dedicated browser test (`grid-truncation-resize-settle`) guards, so it wants its own design pass rather than a reflex edit.
...
Every avenue lands in [`engine/`](engine), the module's pure functional core: a d3-style library of framework-free functions each feature slice consumes, all callable and benchable outside React, which is why the sort and allocation costs above are measurable as pure functions before they are measured as renders. The layering invariant is what keeps them optimizable in isolation — no `'use client'`, no runtime `react` / `motion` / `@dnd-kit` / `@floating-ui` imports, no runtime imports from the module root, no `index` barrel.

[`engine-purity-boundary.test.ts`](../../__tests__/boundary/engine-purity-boundary.test.ts) gates the framework, module-root, and barrel rules for the chat, grid, map, and query engines together — one assertion in code where four ROADMAPs each carried the same paragraph of prose and a copy of the same greps.
...
**See also:** [`__benchmarks__/browser/README.md`](../../__benchmarks__/browser/README.md) (the competitive suite and its methodology) · [`index.ts`](index.ts) (public surface) · [`../../docs/MODULES.md`](../../../docs/MODULES.md). Residual feature work — the editing increments, cell/range selection, nested grouping, variable-height virtualization — is retired from this tracker; the editing design persists in [`../../docs/plans/2026-07-08-GRID-EDITING-PLAN.md`](../../../docs/plans/2026-07-08-GRID-EDITING-PLAN.md) (increments 1–6 and 8 shipped: triggers, cell scope, commit-and-move keys, commit on leave, async commit, undo and redo, and the new-row editor, an internal pinned slot rather than a public row-pinning primitive; the editing benches live in `grid-editing.bench.tsx`), and variable-height virtualization in [`../../docs/plans/2026-08-12-VARIABLE-HEIGHT-VIRTUALIZATION-PLAN.md`](../../../docs/plans/2026-08-12-VARIABLE-HEIGHT-VIRTUALIZATION-PLAN.md) (increment 3 shipped: under `virtualize`, the client-grouped and master-detail bodies window one measured item list each; manual grouping stays unwindowed; the benches live in `grid-window.bench.tsx`). Since #1269 the keyboard cursor walks the client-grouped and master-detail rows; manual grouping still stands it down.
