...
| Feature | Approach |
|---|---|
| Range column-filter types (between / min-max) | `text` / `number` / `select` / `date` / `boolean` ship via the header filter popover; ranges still need a two-bound editor and a `between` operator |
