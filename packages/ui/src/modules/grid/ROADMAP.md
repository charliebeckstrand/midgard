...
Column groups are the tenth, opt-in via `groups` (a plain `GridColumnGroup[]` for the declarative case, or a `{ value, defaultValue, onValueChange }` binding the column manager mutates): a colored, labeled band drawn above a contiguous run of columns. Each group names its member `columns`, a `title`, a `color` from the standard + extended palette (rendered through `Badge`, so the presets match that surface), and optionally a `description` tooltip, an `icon`, and a `collapsible` fold. Groups keep their members contiguous: `groupedColumnOrder` pulls each group's columns together at the first member's position (idempotent, so it re-derives every render), and the header gains a band row above the column headers — one `<th>` per span from `buildGroupSpans`, a `colgroup`-scoped band over each maximal grouped run and a plain filler (which sticks to a pinned column's edge) over the rest — bumping the grid's `aria-rowcount` and the body's global row offset by one. A `collapsible` group folds to its first member behind an expand toggle, hiding the others through a `collapsedHidden` set unioned into the engine's `columnVisibility` (kept apart from the manager's user-hidden set, so collapsing never unchecks a column). When the column manager is on and a `groups` binding is supplied, its dialog's orderable region becomes a group editor: a "New group" button, a zone per group (a name `Input`, a color `Listbox`, a remove button) and an ungrouped pool, with columns dragged between zones (pointer or keyboard, over an isolated dnd-kit `DndContext`) — or moved through a per-row menu as the accessible alternative — to change membership, each edit a pure reducer committed through the binding. The slice is isolated: `grid-group-types`, the `use-grid-group` resolution hook, the `grid-group` band renderer, and the `grid-group-manager` / `use-grid-group-manager` editor. Pinned columns stay out of a band (they partition to an edge, away from their group); multi-level (nested) groups remain on the backlog.
...
| Feature | Approach |
|---|---|
| Column groups / multi-level headers | Single-level shipped — a `groups` band over a contiguous run of columns (title, palette color, collapsible, description, icon), kept contiguous by `groupedColumnOrder`, editable in the column manager. Remaining: nested (multi-level) groups via `getHeaderGroups()` depth |
