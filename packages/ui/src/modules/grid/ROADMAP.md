...
Truncation is the seventh, and on by default (`truncate={false}` opts out, as the editable variant does): overflowing cell content — and column titles — clip to one line with an ellipsis instead of spilling across neighbours, visible wherever the column width is bounded (a resizable/fixed-layout grid). A truncated cell reveals its full text through the native `title` attribute — no floating-ui portal per cell, so a large grid doesn't flood the DOM; a column's `cellTooltip` opts that column into a styled hover/focus `Tooltip` instead (returning a node to show, or `null` to disable the reveal), paying the portal cost only where asked. A truncated header title reveals itself through `title` the same way, the title shrinking within the header's flex slot — and, when sortable, within the sort button — so the ellipsis engages around the sort/filter/resize chrome rather than the title overrunning the cell. Both surfaces share one overflow detector (`useGridTruncation`), measured eagerly at sub-pixel precision.
