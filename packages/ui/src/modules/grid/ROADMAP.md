...
An empty grid (no rows to act on, including while loading) stands its column interactions down: the sort, resize, filter, and reorder affordances drop from the header and the right-click menu defers to the browser's, since each acts on rows that aren't there. The column-manager toolbar — a deliberate tool, not a header affordance — stays available.
