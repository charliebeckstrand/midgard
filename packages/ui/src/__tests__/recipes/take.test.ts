...
describe('take.combobox', () => {
	it('positions the icon 1px inside the control border', () => {
		...
		expect(take.combobox.icon).toBe('absolute inset-y-px right-px')
	})

	it('absolutely positions the icon', () => {
		expect(take.combobox.icon).toMatch(/(^|\s)absolute(\s|$)/)
	})
})
...
describe('take.listbox', () => {
	it('places the icon flush to the button border', () => {
		...
		expect(take.listbox.icon).toBe('absolute inset-y-0 right-0')
	})

	it('absolutely positions the icon', () => {
		expect(take.listbox.icon).toMatch(/(^|\s)absolute(\s|$)/)
	})
})
