...
//
// `kata/tag-input.ts` pads a chip's leading side to `badge.px + bare.p` so the
// label sits symmetric with the remove-button glyph (rationale lives there).
// This pins that sum against the live badge px scale and bare-button compound
// across the sizes a chip takes (xs/sm/md); if either drifts, the assertion
...
describe('tag-input chip leading-pad symmetry', () => {
	for (const size of CHIP_SIZES) {
		const badgeClasses = badge.config.variants.size?.[size] as readonly unknown[]

		const badgePx = findSpacing(badgeClasses, 'px-[')

		const bareP = findBareCompoundP(size)

		const expected = badgePx + bareP

		it(`${size}: chip leading pad = badge.px (${badgePx}) + bare remove-button p (${bareP}) = ${expected}`, () => {
