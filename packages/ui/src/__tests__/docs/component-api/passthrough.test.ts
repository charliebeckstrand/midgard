...
import { detectPassThroughs } from '../../../docs/component-api/passthrough'
...
describe('detectPassThroughs — ComponentPropsWithoutRef-style', () => {
	it('detects a direct ComponentPropsWithoutRef pass-through', () => {
		...
		expect(detectPassThroughs(node, checker)).toEqual([{ element: 'button' }])
	})
	...
	it('detects ComponentPropsWithRef variants too', () => {
		...
		expect(detectPassThroughs(node, checker)).toEqual([{ element: 'a' }])
	})
})

describe('detectPassThroughs — HTMLAttributes-style', () => {
	it('extracts the HTML tag from HTMLDivElement', () => {
		...
		expect(detectPassThroughs(node, checker)).toEqual([{ element: 'div' }])
	})
	...
	it('maps overridden class names to their canonical tag (HTMLHeadingElement → h1)', () => {
		...
		expect(detectPassThroughs(node, checker)).toEqual([{ element: 'h1' }])
	})
...
describe('detectPassThroughs — Omit + Intersection', () => {
	...
	it('skips Pick — pass-through inside Pick is not what we want to surface', () => {
		...
		expect(detectPassThroughs(node, checker)).toEqual([])
	})
