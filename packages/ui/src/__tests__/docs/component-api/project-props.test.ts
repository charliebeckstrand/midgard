...
import { collectProjectPropNames } from '../../../docs/component-api/project-props'
...
function projectNames(sources: Record<string, string>, alias: string): Set<string> {
	...
	return collectProjectPropNames(firstTypeAlias(sf, alias), program.checker)
}

describe('collectProjectPropNames', () => {
	...
	it('drops the entire branch when an alias resolves to a pass-through reference', () => {
		...
		// Pass-through arm is dropped; only the project-authored intersection
		// keys survive.
		expect([...names]).toEqual(['size'])
	})
