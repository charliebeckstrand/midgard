...
import { DragHandleContext } from '../../primitives/drag-handle'
...
describe('chart legend inert prop', () => {
	...
	it('inerts the legend on its own inside a tile being arranged', () => {
		// A drag-handle host is the arranging-tile signal the chart's hover standby
		// already reads; the legend takes the same standby with no prop.
		const { container } = renderUI(
			<DragHandleContext value={{ handle: <span />, claim: () => () => {} }}>
				{chart('bottom')}
			</DragHandleContext>,
		)
