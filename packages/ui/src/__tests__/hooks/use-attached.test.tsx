...
import { useAttached } from '../../components/attached'
import { allBySlot, renderUI } from '../helpers'

describe('useAttached', () => {
	function Group({
		count,
		...
		return <div>{useAttached(children, orientation)}</div>
	}

	it('returns the single child as "only"', () => {
		const { container } = renderUI(<Group count={1} />)

		const [child] = allBySlot(container, 'child')

		expect(child).toHaveAttribute('data-attached', 'only')
	})

	it('marks the first as "start" and the last as "end" for two children', () => {
		const { container } = renderUI(<Group count={2} />)
	...
	it('fills "middle" for everything between first and last', () => {
		const { container } = renderUI(<Group count={5} />)
	...
	it('stamps the orientation on every child', () => {
		const { container } = renderUI(<Group count={3} orientation="vertical" />)
	...
	it('preserves the original key when present', () => {
		const { container } = renderUI(<Group count={3} />)
