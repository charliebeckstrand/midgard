...
import { useGridEditableDraft } from '../../modules/grid/use-grid-editable-draft'
...
function setup(overrides: Overrides = {}) {
	...
	const mutations: GridEditableMutationsApi = { applyCellWrite, applyBulkFill }

	const partialRows: Partial<GridEditableRowsApi<Row>> = {
		rowsRef: {
			current: [
				{ id: 'a', value: 'a1' },
				{ id: 'b', value: 'b1' },
		...
		editableCols: cols,
		getKey: (r: Row) => r.id,
		formatCell: (row: Row, col: GridEditableColumn<Row>) => String(row[col.id as keyof Row] ?? ''),
		parseValue: (raw: string) => raw,
	}

	const rowsApi = partialRows as GridEditableRowsApi<Row>

	const wrapperRef = { current: wrapper } as RefObject<HTMLTableElement | null>

	const { result } = renderHook(() =>
		useGridEditableDraft<Row>({ nav, mutations, rows: rowsApi, wrapperRef }),
	)
...
describe('useGridEditableDraft: beginEdit', () => {
...
describe('useGridEditableDraft: commitEdit', () => {
...
describe('useGridEditableDraft: cancelEdit', () => {
