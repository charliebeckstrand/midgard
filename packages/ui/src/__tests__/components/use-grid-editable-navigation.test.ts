...
import { cellKey, useGridEditableNavigation } from '../../modules/grid/use-grid-editable-navigation'

function setup(rowCount = 3, editableColCount = 3) {
	return renderHook(() => {
		const rowsRef = useRef<number[]>(Array.from({ length: rowCount }, (_, i) => i))

		return useGridEditableNavigation({ rowsRef, editableColCount })
	})
}
...
describe('useGridEditableNavigation: initial state', () => {
...
describe('useGridEditableNavigation: moveActiveTo', () => {
...
describe('useGridEditableNavigation: moveActive', () => {
...
describe('useGridEditableNavigation: moveActiveTab', () => {
...
describe('useGridEditableNavigation: addCellToSelection', () => {
