...
|`INV-API-REFERENCES`|Type references resolve recursively: every PascalCase type name in a formatted prop type resolves to its definition (alias → RHS, interface/object → apparent shape one member per line), transitively with a visited set; excludes node_modules/built-ins/recipe-engine internals; PascalCase tokens inside string literals are NOT harvested.|src/__tests__/docs/api-reference/extract-references.test.ts.|
|`INV-API-UI-RENDER`|ApiReference renders ComponentApi[] as an accordion: props/events partitioned (EVENT_PROP regex) and sorted separately; referenced types expand via a non-blocking Sheet; external types show a hover tooltip naming the package; unions split on top-level \| into badges; multi-line aliases render as CodeBlock; pass-through notes below the tables.|Presentational; behavior pinned by components/api-reference/*.|
