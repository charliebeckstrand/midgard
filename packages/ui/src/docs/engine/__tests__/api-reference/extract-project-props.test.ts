...
describe('extractProjectPropNames', () => {
	it.each<[string, string[], string, string[]]>([
		[
			'collects every key from an inline type literal',
			[
		...
		[
			'skips recognized pass-through arms — only project-authored keys survive',
			[
		...
		[
			'collects literal keys listed inside Pick<T, K>',
			[
		...
		[
			'drops the entire branch when an alias resolves to a pass-through reference',
			[
				`import type { ComponentPropsWithoutRef } from 'react'`,
