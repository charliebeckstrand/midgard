	...
	fs.writeFileSync(
		...
		JSON.stringify({
			compilerOptions: {
				...
				// them costs more than everything this file asserts. The per-file
				// report measures about 1.03s against about 285ms, at
				// `--maxWorkers=1` over three runs each (#1118).
				noLib: true,
			},
		}),
	)
