...
A self-contained documentation site for the `ui` package. Each documented module gets one page with three tabs: an **Overview** (the authored prose with a live render of the component beneath it), a **Usage** playground (the same seeded example with its code and complexity/seed controls), and a generated **API reference**. The engine is a library-agnostic Vite plugin under `engine/`; `ui` wires it up in `vite.docs.config.ts`, and the browser chrome (router, sidebar, tabs) lives in `shell/`.
...
A page is **prose only**, and a high-level overview: what the component is, what it does, and when to reach for it. A single `# h1` (the display name), a first paragraph (the description, shown in the sidebar and page header), then a short body of prose.

**Keep props, values, and variants out of the prose.** The API tab lists them from the source's TSDoc and the Usage tab shows them in a live, synthesized example — both discerned from the code, so they never drift. Anything you restate in prose (a variant name, a color, a default) is a second copy that will. Describe the component's purpose and behavior at a level the code doesn't already declare. There is likewise no front matter for title, order, or placement; those derive from the h1 and the path, and pages sort by name.
...
| --- | --- | --- | --- |
| `complexity` | `minimal \| typical \| rich` | `typical` | Baseline synthesis richness (the visitor's `?level=` overrides it). |
...
One sentence on what it is and when to reach for it.

## A facet

A paragraph on this facet — a variant axis, a state, an accessibility
behavior — in prose. No code; the Usage tab renders the example.
````
