...
- `__tests__/boundary/recipe-import-boundary.test.ts` — components, modules, and primitives import values only via `recipes/kata/<name>`, and a kata imports no value from a sibling kata (`map.ts` → `chart.ts` excepted).
