...
- **No sideways imports.** Kata never import values from sibling kata. A type-only import is allowed, and `map.ts` → `chart.ts` is the one named exception (§1). Shared concerns promote by role: shared *data* to a kiso semantic bundle, shared *wiring* to a katakana bridge. `import { k as <name> }` in a component is a signal the archetype belongs in a bridge.
