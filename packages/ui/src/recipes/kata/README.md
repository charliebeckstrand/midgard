...
- **Object-literal kata** (`k = { … }`) — `k` is a plain object. Used when the component has no top-level variants axis but still needs a curated surface (slot fragments, sub-recipes, motion configs, skeleton data). Recipes for individual slots are inner `defineRecipe(...)` callables: `k.button({ size })`, `k.panel({ surface })`. Wrap a tree of class slots in `defineSlots(...)`: it merges each array to one string at module load, so the consumer's per-element `cn` call hits that memo rather than merging again. The helper takes classes only — keep motion configs and skeleton data outside the tree it wraps.
