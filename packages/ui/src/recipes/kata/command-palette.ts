...
export const k = {
	...
	// below. A virtualized listbox is never CSS `:empty` — `VirtualOptions` keeps
	// a wrapper mounted for its scroll-ancestor walk — so both rules also read
	// that wrapper's `data-empty` signal. Written as literals for the scanner.
	list: ['peer', 'empty:hidden', 'has-[[data-slot=virtual-options][data-empty]]:hidden'],
