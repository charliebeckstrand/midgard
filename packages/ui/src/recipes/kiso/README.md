...
| `sou.ts` (層)      | App-level stacking order — the ordered rung ladder (`overlay` / `chrome` / `elevated` / `float` / `toast`) every portalled surface lands on. Component-local `z-*` inside a positioned box stays inline. *Flat file — single coherent concern.* | —                                                                                                                  |
