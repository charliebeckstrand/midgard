...
| time-ago | `use-time-ago-relative-time.ts:58` | hydration-error | `useState(() => new Date())` seeds the SSR clock → hydration mismatch whenever SSR and hydration straddle a unit boundary. | **applied** (clock seeded in a mount effect; no relative text on the first pass) |
...
| calendar | `calendar.tsx:107` | hydration-error | `useMemo(() => new Date(), [])` for `today` — misleading and hydration-unsafe; propagates to all day-cell highlighting. | **applied** (`today` seeded in a mount effect, `Date \| null` threaded through grid/header/picker) |
