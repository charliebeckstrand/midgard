...
| menu | `menu/menu-content.tsx:50` | floating-ui stamps `role="menu"`+id on the wrapper while `PopoverPanel` also sets `role="menu"` → nested menus; trigger `aria-controls` points at the wrapper, not the menuitem container. | See pattern A. |
| combobox | `combobox/combobox-panel.tsx:69` | Outer positioned div gets a second `role="listbox"` whose only child is the real listbox. | Suppress floating-ui popup role. |
...
| range-slider | `slider/range/range-slider.tsx:129,146` | Thumb names hardcoded `"Range start"`/`"Range end"` with no override — every range slider announces the same generic names. | Accept `aria-label`/`aria-labelledby` (or a labels prop) per thumb. |
...
| drawer | `drawer/drawer.tsx:43` | Requires a `DrawerTitle` for its name but exposes no `aria-label` escape hatch → title-less drawer is unnamed. | Accept and forward `aria-label` like `Dialog` does. |
...
| query-builder | `query-builder-rule.tsx:79,93`, `query-builder-rule-value.tsx:24` | Field/operator/value `Select`s pass only `placeholder`; Listbox never names itself from placeholder → unnamed comboboxes. | Pass `aria-label` ("Field"/"Operator"/"Value"). |
...
- **sheet** `sheet/sheet.tsx:48` — title-less sheet is unnamed (no `aria-label` passthrough). Add it.
...
- **tag-input** `tag-input/tag-input.tsx:104` — current tags sit in a bare div with no list/group role, so they aren't enumerable on demand. Add `role="list"` (or labeled `role="group"`).
...
- **link** `link/link.tsx:10` — no new-tab affordance / `rel="noopener"` when `target="_blank"`.
