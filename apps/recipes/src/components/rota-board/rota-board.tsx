...
import type { DayColumn, PlanCard } from '../../utilities/plan-move'
...
/** How a column names itself to a screen reader, which wants the day in full. */
const FULL_DAY: Intl.DateTimeFormatOptions = { weekday: 'long', day: 'numeric', month: 'long' }
...
export function RotaBoard({ days, onMove, onAdd, onCooked, onRemove }: RotaBoardProps) {
	...
	return (
		...
		>
			{days.map((entry) => {
				// Once per column: the same date reads on the header and in the add
				// button's name.
				const date = dayLabel(entry.day, DATE)
				...
				return (
					<KanbanColumn
						...
						aria-label={dayLabel(entry.day, FULL_DAY)}
					>
						<KanbanColumnHeader>
							<Flex justify="between" align="center" className="w-full min-w-0">
								<Flex direction="col" className="min-w-0">
									...
									<Text className="text-xs text-zinc-500 dark:text-zinc-400">{date}</Text>
								</Flex>
